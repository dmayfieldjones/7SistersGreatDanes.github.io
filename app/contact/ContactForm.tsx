...
export default function ContactForm() {
  ...
  const getPreviousDogsText = (): string => {
    ...
    if (part1Data.previousDogsExtraLarge) options.push('Other Extra Large breed(s) - e.g., Mastiff, Saint Bernard, Newfoundland, Great Pyrenees')
    if (part1Data.previousDogsLarge) options.push('Large breed(s) - e.g., German Shepherd, Labrador, Golden Retriever, Boxer')
    if (part1Data.previousDogsMedium) options.push('Medium breed(s) - e.g., Border Collie, Australian Shepherd, Beagle, Bulldog')
    if (part1Data.previousDogsSmall) options.push('Small breed(s) - e.g., Dachshund, Cocker Spaniel, French Bulldog, Poodle')
  ...
  const getPurposeText = (): string => {
    ...
    if (part1Data.purposeOpenToShows) options.push('Open to shows/conformation (may consider in future)')
  ...
  const handlePart1Submit = async (e: React.FormEvent) => {
    ...
    try {
      // Format the email body, handling checkboxes specially
      ...
      // Format "how did you hear" with readable text
      ...
      // Send to Dustin first (primary recipient)
      const formData = new FormData()
      formData.append('email', part1Data.email)
      formData.append('name', `${part1Data.firstName} ${part1Data.lastName}`)
      formData.append('subject', `7Sisters Farm - Inquiry from ${part1Data.firstName} ${part1Data.lastName}`)
      formData.append('message', emailBody)
      formData.append('_cc', part1Data.email)
      formData.append('_replyto', part1Data.email)
      formData.append('_template', 'box')
      formData.append('_captcha', 'false')
      formData.append('_autoresponse', `Thank you for your inquiry to 7Sisters Farm, ${part1Data.firstName}! We've received your information and will be in touch soon.`)
      ...
      // Send to Karen with same data
      const formData2 = new FormData()
      formData2.append('email', part1Data.email)
      formData2.append('name', `${part1Data.firstName} ${part1Data.lastName}`)
      formData2.append('subject', `7Sisters Farm - Inquiry from ${part1Data.firstName} ${part1Data.lastName}`)
      ...
      const response2 = await fetch('https://formsubmit.co/ajax/karenmayfieldjones@gmail.com', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
        },
      ...
      // Check if verification is needed (FormSubmit returns specific messages)
      const needsVerification = result1.message?.includes('verify') || result2.message?.includes('verify') ||
                                result1.message?.includes('confirmation') || result2.message?.includes('confirmation')
      ...
      if (response1.ok && result1.success && response2.ok && result2.success) {
        ...
        // Verification email sent - still show success but inform user
        setPart1Submitted(true)
        setSubmitStatus('success')
        setIsSubmitting(false)
        console.log('Verification emails have been sent. Please check your inbox.')
      } else {
        // If at least one succeeds, still show success but log the error
        if ((response1.ok && result1.success) || (response2.ok && result2.success)) {
          setPart1Submitted(true)
          setSubmitStatus('success')
          setIsSubmitting(false)
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  ...
                  required
                />
                {part1Errors.firstName && <span className="error-message">{part1Errors.firstName}</span>}
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  required
                />
                {part1Errors.lastName && <span className="error-message">{part1Errors.lastName}</span>}
              </div>
            </div>
            <div className="form-row">
              <div className="form-group">
          ...
          <div className="form-section">
            ...
            <div className="form-group">
              ...
              {part1Data.purposeDogSports && (
                <div className="form-group" style={{ marginTop: '0.75rem', marginLeft: '1.5rem', paddingLeft: '1rem', borderLeft: '3px solid #bf141c' }}>
                  <label htmlFor="purposeDogSportsDetails" style={{ fontSize: '0.95rem', fontWeight: '500' }}>Which dog sports? (optional)</label>
                  <input
                    type="text"
              ...
              {part1Data.purposeOther && (
                <div className="form-group" style={{ marginTop: '0.75rem', marginLeft: '1.5rem', paddingLeft: '1rem', borderLeft: '3px solid #bf141c' }}>
                  <label htmlFor="purposeOtherDetails" style={{ fontSize: '0.95rem', fontWeight: '500' }}>Please specify other purpose</label>
                  <input
                    type="text"
              ...
              <div className="checkbox-group">
                ...
                <label className="checkbox-label">
                  <input
                    ...
                    onChange={handlePart1Change}
                  />
                  <span>Other Extra Large breed(s) - e.g., Mastiff, Saint Bernard, Newfoundland, Great Pyrenees</span>
                </label>
                <label className="checkbox-label">
                  <input
                    type="checkbox"
                    ...
                    onChange={handlePart1Change}
                  />
                  <span>Large breed(s) - e.g., German Shepherd, Labrador, Golden Retriever, Boxer</span>
                </label>
                <label className="checkbox-label">
                  <input
                    type="checkbox"
                    ...
                    onChange={handlePart1Change}
                  />
                  <span>Medium breed(s) - e.g., Border Collie, Australian Shepherd, Beagle, Bulldog</span>
                </label>
                <label className="checkbox-label">
                  <input
                    type="checkbox"
                    ...
                    onChange={handlePart1Change}
                  />
                  <span>Small breed(s) - e.g., Dachshund, Cocker Spaniel, French Bulldog, Poodle</span>
                </label>
                <label className="checkbox-label">
                  <input
                    type="checkbox"
          ...
          <div className="form-submit-section">
            {submitStatus === 'error' && (
              <div className="error-message-box">
                <p><strong>There was an error submitting your inquiry.</strong></p>
                <p style={{ marginTop: '0.5rem' }}>
                  This might be because the email addresses need to be verified with FormSubmit first. 
                  Please check the inboxes for <strong>dustin@mayfieldjones.com</strong> and <strong>karenmayfieldjones@gmail.com</strong> 
                  for verification emails, or email us directly at{' '}
